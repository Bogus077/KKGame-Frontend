...
export const KKBaseQuery = fetchBaseQuery({
  baseUrl: API_URL,
...
export const kkBaseQueryWithReAuth: BaseQueryFn<
  string | FetchArgs,
  unknown,
  FetchBaseQueryError
> = async (args, api, extraOptions) => {
  await mutex.waitForUnlock();
  let result = await KKBaseQuery(args, api, extraOptions);
  if (
    result.error &&
    ...
    if (!mutex.isLocked()) {
      ...
      try {
        ...
        const refreshResult = (await KKBaseQuery(
          {
            url: '/auth/refresh',
          },
          refreshApi,
        ...
        if (refreshResult.data) {
          ...
          result = await KKBaseQuery(args, api, extraOptions);
        } else {
          api.dispatch(logOut());
        }
      } finally {
